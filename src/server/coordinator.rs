...
/// before it is evicted for stalling even though it keeps heartbeating (issue #54).
...
impl ConsumerGroup {
    ...
    /// "cooperative", such as `cooperative-sticky`) rather than an
...
impl GroupCoordinator {
    ...
    pub fn join_group(
        ...
        // member to (re)start the group picks it, following the convention that clients
        // list their preferred assignor first.
        if was_empty {
    ...
    /// wire-protocol path for a follower to ever learn its own assignment at all. Now
    ...
    pub fn heartbeat(
        ...
        if let Some(group) = groups.get_mut(group_id) {
            ...
            if generation_id != group.generation_id {
                ...
                // recognizable "REBALANCE_IN_PROGRESS" prefix that clients key off
